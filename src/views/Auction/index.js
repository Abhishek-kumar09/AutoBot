import { Button, Card, CardActions, Grid, Hidden, makeStyles, Typography } from '@material-ui/core';
...
export default function Auction({ user }) {
  ...
  const fetchFromFirestore = () => {
    const dataArray = []
    firestore.collection("auctionData").get().then((snapshot) => {
  ...
  return (
    <div className={classes.root}>
      ...
      <div style={{ padding: '32px', marginTop: '40px' }}>
            ...
            <Grid item xs={12} md={4} lg={3} style={{ margin: '45px 0px' }}  >
              <Card elevation={10} style={{ height: '100%', display: 'flex', flexDirection: 'column', backgroundColor: '#E5F6F2' }}>
                <div style={{ padding: '16px', backgroundColor: `#0${index % 10}AE86`, color: '#fff' }}>
                  <Typography align="left" variant="h4">{item.dbname}</Typography>
                </div>
                <CardBody style={{ flexGrow: 1 }}>
                  ...
                  {item.topic !== "Others" ? <Typography className={classes.typography}>About: {item.topic}</Typography> : ''}
                  <Typography className={classes.typography}>By: {item.name}</Typography>
                  <Typography className={classes.typography}>Dataset Size: {item.dataset_size}</Typography>
                </CardBody>
                <div>
                  <CardBody>
                  ...
                  <CardActions style={{ margin: '10px 18px' }}>
                    <Button fullWidth className="round block accent" href={item.link} target="_blank">Request Claim {item.download_size}</Button>
                  </CardActions>
                </div>
              </Card>
            </Grid>
          ...
          <Grid item xs={12} md={4} lg={3} style={{ margin: '45px 0px' }}  >
            <HashLink smooth to="#form">
              <Card elevation={16}
                ...
                style={{ height: '100%', display: 'flex', flexDirection: 'column', backgroundColor: '#E5F6F2', padding: '40px 30px', minHeight: '400px', textAlign: 'center', justifyContent: 'space-around' }}>
                <img src="/icons/plus.svg" alt="" height="200px"></img>
                <Typography variant="h6">
                  Add your data for auction
                </Typography>
              </Card>
            </HashLink>
          </Grid>
        ...
        <Hidden xsUp={hide}>
          <SnackbarProvider>
            <Form id="form" title=" Get your Data Auctioned by the rest" user={user} hide={hide} setHide={setHide} />
          </SnackbarProvider>
        </Hidden>
      </div>
    </div>
