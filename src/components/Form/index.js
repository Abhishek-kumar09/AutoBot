...
function Form({ ...rest }) {
  ...
  const handleSubmit = e => {
    ...
    firestore.collection("auctionData").doc(`${formData.email}_${formData.dbname}`)
      ...
      .then(() => {
        setSubmitting(0);
        enqueueSnackbar('Your Data is received to us and we are grinding it for a purpose', { variant: 'success' });
        setTimeout(() => {
    ...
    <Paper elevation={20} className={classes.root} {...rest}>
      <Container className={classes.container}>
        ...
        <Typography
          ...
          variant="subtitle2"
        >
          All the data is first verified and then publicly visible, you can ofcourse hide it from public
        </Typography>
        ...
        <ValidatorForm onSubmit={handleSubmit}>
          ...
          <div style={{ padding: '30px 0 15px' }}>
            <Typography style={{ margin: 0 }} variant="caption" color="initial">
              By continuing, you agree to the Autobot Terms of Use and
              Privacy Policy
            </Typography>
          </div>
